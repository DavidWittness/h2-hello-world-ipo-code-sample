...
function CartHeader() {
  return (
    <Link
      to="/cart"
      className="relative ml-auto flex items-center justify-center w-8 h-8 ml-auto"
    >
      <svg
...
function CartCount() {
  ...
  const totalQuantity = root.data?.cart?.totalQuantity;
  return (
    <>
      {totalQuantity > 0 && (
        <div className="text-contrast bg-red-500 text-white absolute bottom-1 right-1 text-[0.625rem] font-medium subpixel-antialiased h-3 min-w-[0.75rem] flex items-center justify-center leading-none text-center rounded-full w-auto px-[0.125rem] pb-px">
          <span>{root.data?.cart?.totalQuantity}</span>
        </div>
      )}
    </>
  );
